...
const CategoryPage = () => {
  ...
  useEffect(() => {
    if (allPages.length > 0) {
      const shopPage = allPages.find((p: any) => p.slug === "shop" || p.route_path === "/shop");
      if (shopPage) {
        dispatch(setCurrentPages(shopPage));
      }
    }
  }, [allPages, dispatch]);
  ...
  const cmsFilters = useMemo(() => {
    const shopPage = allPages.find((p: any) => p.slug === "shop" || p.route_path === "/shop");
    if (!shopPage) return defaultSidebarFilters;
    return shopPage.content?.filter((section: any) => section.layout === "sidebar") || defaultSidebarFilters;
  }, [allPages]);
